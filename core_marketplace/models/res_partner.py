...
class ResPartner(models.Model):
    ...
    def get_mention_suggestions(self, search, limit=8, channel_id=None):
        ...
        if not self.env.user.has_group ('core_marketplace.marketplace_officer_group'):
           return []
    ...
    def _set_payment_method(self):
        ...
        try:
            payment_method_cheque_id = self.env['ir.model.data']._xmlid_to_res_id(
                'core_marketplace.marketplace_seller_payment_method_data2')
            if payment_method_cheque_id:
                return_list.append(payment_method_cheque_id)
        except Exception as e:
            _logger.warning("Warning! cheque seller payment method not found~%r",e)
            pass
        try:
            payment_method_bank_transfer_id = self.env['ir.model.data']._xmlid_to_res_id(
                'core_marketplace.marketplace_seller_payment_method_data5')
            if payment_method_bank_transfer_id:
                return_list.append(payment_method_bank_transfer_id)
        except Exception as e:
    ...
    def _default_website_sequence(self):
        min_seq_record = self.search([('website_sequence', '!=', False)], order='website_sequence asc', limit=1)
        if min_seq_record:
            return min_seq_record.website_sequence - 1
        return 10
    ...
    color = fields.Integer(string="Color Index", default=0)  # For kanban view coloring
    ...
    hide_notebook =fields.Boolean("hide notebook",compute="_compute_hide_notebook")
    ...
    # Odoo 18: _read_group_fill_results removed
    # @api.model
    ...
    def write(self, vals):
        new_seller = False
        # Guard: env.user may be empty with auth='none' or during user creation
        is_manager = (
            self.env.su
            or not self.env.user
            or self.env.user.has_group('core_marketplace.marketplace_manager_group')
        )
        if not is_manager:
        ...
        for rec in self:
            if rec.seller and change_state_to == "approved":
                ...
                # Auto-create wallet for newly approved seller
                self.env['seller.wallet'].sudo().get_or_create_wallet(rec.id)
    ...
    def action_seller_sol(self):
        ...
        self.ensure_one()
        action = self.env.ref('core_marketplace.wk_seller_slae_order_line_action')
        list_view_id = self.env['ir.model.data']._xmlid_to_res_id(
            'core_marketplace.wk_seller_product_order_line_tree_view')
        form_view_id = self.env['ir.model.data']._xmlid_to_res_id(
            'core_marketplace.wk_seller_product_order_line_form_view')
        return {
            'name': action.name,
            'help': action.help,
            'type': action.type,
    ...
    def action_seller_globel_settings(self):
        ...
        self.ensure_one()
        form_view_id = self.env['ir.model.data']._xmlid_to_res_id(
            'core_marketplace.mp_seller_globel_config_form_view')
        return {
            ...
            'context' : {'module' : 'core_marketplace'},
        }
    ...
    def change_seller_group(self, set_to_group=None):
        ...
        if not login_user_obj.has_group('core_marketplace.marketplace_officer_group'):
            _logger.warning(_("Warning!! You are not an autorized user to change seller account access. Please contact your administrator. "))
        ...
        for seller in self.filtered(lambda o: o.seller == True):
            ...
            pending_seller_group_obj = self.env.ref('core_marketplace.marketplace_draft_seller_group')
            seller_group_obj = self.env.ref('core_marketplace.marketplace_seller_group')
            if set_to_group == "seller":
                #First check seller user realy belongs to draft seller group(marketplace_draft_seller_group) or not
                if seller_user.has_group("core_marketplace.marketplace_draft_seller_group"):
                ...
                if seller_user.has_group("core_marketplace.marketplace_seller_group"):
    ...
    def action_avg_seller_review_fun(self):
        ...
        self.ensure_one()
        action = self.env.ref('core_marketplace.action_seller_review')
        list_view_id = self.env['ir.model.data']._xmlid_to_res_id(
            'core_marketplace.mp_seller_review_tree_view_webkul2')
        form_view_id = self.env['ir.model.data']._xmlid_to_res_id(
            'core_marketplace.mp_seller_review_form_view_webkul2')
        return {
            'name': action.name,
            'help': action.help,
            'type': action.type,
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        manager_group = self.env['ir.model.data']._xmlid_to_res_id(
    ...
    def register_partner_as_a_seller(self):
        ...
        for rec in self:
            ...
            return {
                ...
                'view_id':rec.env.ref('core_marketplace.seller_registration_wizard_form').id,
                'context' : context,
                'target':'new',
            }
