...
class ProductTemplate(models.Model):
    ...
    activity_date_deadline = fields.Date(
        ...
        groups="base.group_user,core_marketplace.marketplace_seller_group")
    ...
    def mp_action_view_sales(self):
        """ Method used in other marketplace module"""
        self.ensure_one()
        action = self.env.ref('core_marketplace.wk_seller_slae_order_line_action')
        product_ids = self.product_variant_ids.ids
        tree_view_id = self.env.ref('core_marketplace.wk_seller_product_order_line_tree_view')
        form_view_id = self.env.ref('core_marketplace.wk_seller_product_order_line_form_view')
        return {
            'name': action.name,
            'help': action.help,
            'type': action.type,
            ...
            if obj.marketplace_seller_id.state == "approved":
                if obj.product_variant_ids:
                    if obj.marketplace_seller_id.get_seller_global_fields('auto_product_approve'):
                        ...
                        form_view_ref = self.env.ref('core_marketplace.wk_product_variant_wizard_view')
                        return {
    ...
    def _get_combination_info(self, combination=False, product_id=False, add_qty=1.0, pricelist=False, parent_combination=False, only_template=False, **kwargs):
        # Note: pricelist parameter removed in Odoo 18, kept for backwards compatibility
