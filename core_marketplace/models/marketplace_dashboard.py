...
class marketplace_dashboard(models.Model):
    ...
    def is_user_seller(self):
        ...
        seller_group = self.env['ir.model.data']._xmlid_to_res_id(
    ...
    def _is_seller_or_manager(self):
        for rec in self:
            ...
            if current_user:
                seller_groups = self.env.ref('core_marketplace.marketplace_seller_group')
                manager_group = self.env.ref('core_marketplace.marketplace_officer_group')
                if current_user.id in seller_groups.users.ids:
                    is_seller = True
                if current_user.id in manager_group.users.ids:
                    is_seller = False
            ...
            if rec.state == 'product':
                if rec.is_user_seller():
                    obj = self.env['product.template'].search(
                        [('marketplace_seller_id.user_ids', '=', self.env.user.id), ('status', '=', 'approved')])
                else:
                    obj = self.env['product.template'].search(
    ...
    def _get_pending_count(self):
        ...
        for rec in self:
            if rec.state == 'product':
                if rec.is_user_seller():
                    obj = self.env['product.template'].search(
                        [('marketplace_seller_id.user_ids', '=', self.env.user.id), ('status', '=', 'pending')])
                else:
                    obj = self.env['product.template'].search(
    ...
    def _get_rejected_count(self):
        ...
        for rec in self:
            if rec.state == 'product':
                if rec.is_user_seller():
                    obj = self.env['product.template'].search(
                        [('marketplace_seller_id.user_ids', '=', self.env.user.id), ('status', '=', 'rejected')])
                else:
                    obj = self.env['product.template'].search(
    ...
    def _get_done_count(self):
        ...
        for rec in self:
            if rec.state == 'order':
                ...
                if rec.is_seller:
                    obj = self.env['sale.order.line'].search(
                        [('marketplace_seller_id', '=',user_obj.partner_id.id), ('marketplace_state', '=', 'done')])
                else:
                    obj = self.env['sale.order.line'].search(
                        [('marketplace_seller_id', '!=', False), ('marketplace_state', '=', 'done')])
