...
class SaleOrder(models.Model):
    ...
    def action_so_done(self):
        ...
        for rec in self:
            if rec.order_line:
                mp_order_line = rec.order_line.filtered(lambda line: line.marketplace_seller_id != False and (line.marketplace_state == "shipped" or (line.marketplace_state == "approved" and line.product_id.type == "service")))
                if mp_order_line:
...
class SaleOrderLine(models.Model):
    ...
    marketplace_state = fields.Selection(STATES, default="new",tracking=True,copy=False)
    mp_delivery_count = fields.Integer(string='Delivery Orders', compute='_compute_seller_picking_ids')
    product_sale_ok = fields.Boolean(string="Sale_ok",related="product_id.sale_ok")
    ...
    pro_image128 = fields.Image("image",related="product_id.image_128")
    ...
    def action_mark_done(self):
        ...
        for rec in self:
            ...
            elif rec.product_id.type == 'service' and rec.marketplace_state =='approved':
                rec.order_id.sudo().action_so_done()
            rec.sudo()._adding_followers()

    def get_mark_done_approval_wizard_action(self):
        """ Open bulk approval wizard """
        # action = self.env["ir.actions.actions"]._for_xml_id("core_marketplace.markdone_details_form_view")
    ...
    def action_view_delivery(self):
        ...
        '''
        action = self.env.ref('core_marketplace.marketplace_stock_picking_action').sudo().read()[0]
        ...
        if len(pickings) > 1:
            ...
            action['views'] = [(self.env.ref('core_marketplace.marketplace_picking_stock_modified_form_view').id, 'form')]
    ...
    def action_view_sol_seller_payment(self):
        ...
        action = self.env.ref('core_marketplace.wk_seller_payment_action').sudo().read()[0]
        if len(seller_payment_objs) > 1:
            action['domain'] = [('id', 'in', seller_payment_objs.ids)]
        elif len(seller_payment_objs) == 1:
            action['views'] = [(self.env.ref('core_marketplace.wk_seller_payment_form_view').id, 'form')]
