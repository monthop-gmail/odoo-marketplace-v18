...
class AccountMove(models.Model):
    ...
    seller_invoice_number = fields.Char(
        string='Seller Invoice Number', translate=True)
    ...
    def create_seller_payment_new(self, sellers_dict):
        ...
        for seller_id in list(sellers_dict.get("seller_ids")):
            ...
            try:
                ...
                if seller_payment_obj:
                    ...
                    if seller_payment_obj.payment_mode == 'order_paid':
                        self._credit_seller_wallet(
                            seller_id,
                            abs(seller_payment_obj.payable_amount),
                            sellers_dict.get('memo', ''),
                            seller_payment_obj.id,
                        )
            except Exception as e:
                _logger.warning("Exception~%r~",e)
                pass
        ...
        if sellers_dict:
            ...
            for seller in sellers_dict["seller_ids"].keys():
                ...
                # Credit seller wallet for order_paid
                if sellers_dict.get('payment_mode') == 'order_paid':
                    payable = invoice_currency.compute(total_amount, mp_currency_obj)
                    self._credit_seller_wallet(
                        seller, abs(payable),
                        sellers_dict.get('memo', ''),
                        seller_payment_id.id,
                    )

    @api.model
    def _credit_seller_wallet(self, seller_id, amount, reference, seller_payment_id):
        """Credit seller wallet when commission payment is confirmed."""
        try:
            wallet = self.env['seller.wallet'].sudo().get_or_create_wallet(seller_id)
            if wallet and wallet.state == 'active' and amount > 0:
                wallet.credit(
                    amount=amount,
                    transaction_type='commission_credit',
                    description=_('Commission from order: %s') % reference,
                    reference=reference,
                    seller_payment_id=seller_payment_id,
                )
        except Exception as e:
            _logger.warning('Failed to credit wallet for seller %s: %r', seller_id, e)
