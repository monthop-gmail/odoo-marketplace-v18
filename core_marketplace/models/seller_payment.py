...
class SellerPayment(models.Model):
            ...
            if rec.payment_mode == "order_paid":
                if rec.memo:
                    ...
                    if all(move.state == 'done' for move in stock_move_objs):
                        flag = True
                # Need to fix the below part in case of automatic invoice
    ...
    name = fields.Char(string="Record Reference",
                       default="NEW", translate=True, copy=False)
    ...
    seller_id = fields.Many2one("res.partner", string="Seller", domain=[
                                ('seller', '=', True)], required=True, copy=False)
    ...
    payment_type = fields.Selection([('cr', 'Credit'), ('dr', 'Debit')], default="cr", string="Type",
                                    help="Credit means vendor will pay you & debit means vendor has been paid.", copy=False)
    payment_mode = fields.Selection([('order_paid', 'Order Paid'), ('order_refund', 'Order Refund'), (
        'seller_payment', 'Seller Payment')], string="Payment Mode", required=True, default="order_paid", copy=False)
    invoice_id = fields.Many2one("account.move", string="Invoice", copy=False)
    payable_amount = fields.Float(string="Payable Amount", required=True,
                                  help="Positive amount means vendor will pay to seller & negative amount means vendor has been paid/or seller have to refund.", copy=False)
