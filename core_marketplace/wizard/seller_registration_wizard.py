...
class SellerConfirmation(models.TransientModel):
	...
	def get_seller_profile(self):
		...
		var_url = '/web#id='+ str(self.partner_id.id) + '&view_type=form&model=res.partner&menu_id='+str(self.env.ref('core_marketplace.wk_seller_dashboard').id)+'&action='+str(self.env.ref('core_marketplace.wk_seller_action').id)
	...
	def confirm_customer_as_seller(self):
		...
		try:
			...
			draft_seller_group_id = self.env['ir.model.data']._xmlid_to_res_id('core_marketplace.marketplace_draft_seller_group')
			...
			grp_portal = self.env['ir.model.data']._xmlid_to_res_id('base.group_portal')
			grp_portal_obj = self.env['res.groups'].browse(grp_portal)
			grp_portal_obj.write({'users':[(3,current_user_id.id)]})
			grp_internal = self.env['ir.model.data']._xmlid_to_res_id('base.group_user')
		...
		return {
			...
			'res_id' : self.id,
			'view_id':self.env.ref('core_marketplace.registration_completed_wizard_form').id,
