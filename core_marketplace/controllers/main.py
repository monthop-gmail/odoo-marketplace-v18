...
class AuthSignupHome(Website):
    ...
    def web_login(self, redirect=None, *args, **kw):
        ...
        if request.params['login_success']:
            ...
            if not current_user.has_group('base.group_user') and current_user.has_group('core_marketplace.marketplace_draft_seller_group') and current_user.partner_id.seller:
                seller_dashboard_menu_id = request.env['ir.model.data']._xmlid_to_res_id('core_marketplace.wk_seller_dashboard')
    ...
    def seller_signup_form(self, *args, **kw):
        ...
        return request.render('core_marketplace.mp_seller_signup', qcontext)
...
class website_marketplace_dashboard(http.Controller):
    ...
    def mp_terms_and_conditions(self,**post):
        ...
        return request.env['ir.qweb']._render('core_marketplace.mp_t_and_c_modal_template', {'mp_t_and_c':mp_t_and_c,})
    ...
    def become_seller(self, **post):
        ...
        if current_user.has_group("core_marketplace.marketplace_officer_group"):
            grp_name = "Manager" if current_user.has_group("core_marketplace.marketplace_manager_group") else "Officer"
        ...
        return request.render('core_marketplace.convert_user_into_seller',values)
    ...
    def submit_as_seller(self, **post):
        ...
        if not seller_detail and country_id and url_handler:
            ...
            draft_seller_group_id = request.env['ir.model.data'].sudo()._xmlid_to_res_id('core_marketplace.marketplace_draft_seller_group')
    ...
    def account(self):
        ...
        seller_dashboard_menu_id = request.env[
            'ir.model.data']._xmlid_to_res_id('core_marketplace.wk_seller_dashboard')
...
class MarketplaceSellerProfile(http.Controller):
    ...
    def seller(self, seller_id=None, seller_url_handler=None, page=0, category=None, search='', ppg=False, **post):
        ...
        if seller and not seller.website_published:
            officer_group = env.ref('core_marketplace.marketplace_officer_group')
            manager_group = env.ref('core_marketplace.marketplace_manager_group')
            user_obj = env.user
        ...
        product_count = request.env["product.template"].sudo().search_count([('sale_ok', '=', True), ('status', '=', "approved"), ("is_published", "=", True), ("id", "in", seller_product_ids.ids)])
        pager = request.website.pager(url=url, total=product_count, page=page, step=ppg, scope=7, url_args=post)
        products = env['product.template'].sudo().search([('sale_ok', '=', True), ('status', '=', "approved"), ("is_published", "=", True), ("marketplace_seller_id", "=", seller.id)], limit=ppg, offset=pager['offset'], order='website_sequence desc')
        ...
        return request.render("core_marketplace.mp_seller_profile", values)
    ...
    def seller_profile_recently_product(self, seller_id, page=0, category=None, search='', ppg=False, **post):
        ...
        return request.env['ir.qweb']._render('core_marketplace.shop_recently_product', values)
    ...
    def _get_seller_search_domain(self, search):
        domain = [("is_published", "=", True), ("seller", '=', True), ("state", "=", "approved")]
        if search:
    ...
    def load_mp_all_seller(self, page=0, search='', ppg=False, **post):
        ...
        return request.render("core_marketplace.sellers_list", values)
...
class MarketplaceSellerShop(http.Controller):
    ...
    def seller_shop(self, shop_url_handler, page=0, category=None, search='', ppg=False, **post):
        ...
        product_count = request.env["product.template"].sudo().search_count([('sale_ok', '=', True), ('status', '=', "approved"), ("is_published", "=", True), ("id", "in", shop_obj.sudo().seller_product_ids.ids)])
        pager = request.website.pager(url=url, total=product_count, page=page, step=ppg, scope=7, url_args=post)
        products = env['product.template'].sudo().search([('sale_ok', '=', True), ('status', '=', "approved"), ("is_published", "=", True), ("marketplace_seller_id", "=", shop_obj.sudo().seller_id.id), ("id", "in", shop_obj.sudo().seller_product_ids.ids)], limit=ppg, offset=pager['offset'], order='website_sequence desc')
        ...
        return request.render("core_marketplace.mp_seller_shop", values)
    ...
    def seller_shop_recently_product(self, shop_id, page=0, category=None, search='', ppg=False, **post):
        ...
        recently_product_obj = request.env['product.template'].search([('sale_ok', '=', True), ('status', '=', "approved"), ("is_published", "=", True), (
    ...
    def mp_sell(self, redirect=None, **post):
        ...
        return request.render("core_marketplace.wk_mp_seller_landing_page", values)
    ...
    def load_mp_all_seller_shop(self, page=0, search='', ppg=False, **post):
        ...
        return request.render("core_marketplace.seller_shop_list", values)
...
class SellerReview(http.Controller):
    ...
    def load_seller_review(self, seller_id, offset=0, limit=False, sort_by="recent", filter_by=-1, **kwargs):
        ...
        return request.env['ir.qweb']._render('core_marketplace.wk_seller_review_template', values)
    ...
    def load_seller_review_count(self, seller_id, offset=0, limit=False, sort_by="recent", filter_by=-1, **kwargs):
        ...
        if filter_by == -1:
            review_ids = request.env["seller.review"].search([('marketplace_seller_id', '=', seller_id), (
                'is_published', '=', True)], order="helpful desc" if sort_by == "most_helpful" else "create_date desc")
        if filter_by == 1:
            review_ids = request.env["seller.review"].search([('marketplace_seller_id', '=', seller_id), ('is_published', '=', True), (
...
class TrackSol(http.Controller):
    ...
    def track_order_line(self, sol_id, **kwargs):
        ...
        return request.env['ir.qweb']._render('core_marketplace.marketplace_order_line_info', values)
...
class MarketplaceMail(http.Controller):
    ...
    def mp_chatter_json(self, res_model='', res_id=None, message='', **kw):
        ...
        if data:
            return request.env['ir.qweb']._render('core_marketplace.mp_chatter_mail_append', data)
        else:
            return False
...
class PortalAccount(CustomerPortal):
    ...
    def portal_my_invoices(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, **kw):
        ...
        if request.env.user.has_group('core_marketplace.marketplace_manager_group') or request.env.user.has_group('core_marketplace.marketplace_officer_group'):
          domain = self._get_invoices_domain()
        else:
